...
#include <StarEngine/Renderer/DeviceManagerVulkan.h>
...
namespace StarEngine {
	...
	bool DeviceManagerVulkan::createInstance()
	{
		if (!m_DeviceParams.headlessDevice)
		{
			if (!glfwVulkanSupported())
			{
				SE_CORE_ERROR("GLFW reports that Vulkan is not supported. Perhaps missing a call to glfwInit()?");
				return false;
			}
		...
		if (!requiredExtensions.empty())
		{
			...
			SE_ERROR("%s", ss.str().c_str());
			return false;
		}

		Log::LogMessage(m_DeviceParams.infoLogSeverity, "Enabled Vulkan instance extensions:");
		for (const auto& ext : enabledExtensions.instance)
		{
			Log::LogMessage(m_DeviceParams.infoLogSeverity, "    %s", ext.c_str());
		}
		...
		Log::LogMessage(m_DeviceParams.infoLogSeverity, "Enabled Vulkan layers:");
		for (const auto& layer : enabledExtensions.layers)
		{
			Log::LogMessage(m_DeviceParams.infoLogSeverity, "    %s", layer.c_str());
		}
		...
		{
			SE_ERROR("Call to vkEnumerateInstanceVersion failed, error code = %s", nvrhi::vulkan::resultToString(VkResult(res)));
			return false;
		}
		...
		{
			SE_ERROR("The Vulkan API version supported on the system (%d.%d.%d) is too low, at least %d.%d.%d is required.",
		...
		{
			SE_ERROR("The Vulkan API supported on the system uses an unexpected variant: %d.", VK_API_VERSION_VARIANT(applicationInfo.apiVersion));
			return false;
		}
		...
		if (res != vk::Result::eSuccess)
		{
			SE_ERROR("Failed to create a Vulkan instance, error code = %s", nvrhi::vulkan::resultToString(VkResult(res)));
			return false;
		}
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL vulkanDebugCallback(
		...
		void* userData)
	{
		const bool DeviceManagerVulkan::createInstance()* manager = (const DeviceManageVulkan*)userData;
		...
		SE_CORE_WARN("[Vulkan: location=0x%zx code=%d, layerPrefix='%s'] %s", location, code, layerPrefix, msg);

		return VK_FALSE;
	}

	void bool DeviceManagerVulkan::createInstance()
		::installDebugCallback()
	{
		...
		SE_ASSERT(res == vk::Result::eSuccess);
	}

	bool DeviceManagerVulkan::pickPhysicalDevice()
	{
		...
		if (adapterIndex >= 0)
		{
			...
			{
				SE_ERROR("The specified Vulkan physical device %d does not exist.", adapterIndex);
				return false;
			}
		...
		for (int deviceIndex = firstDevice; deviceIndex <= lastDevice; ++deviceIndex)
		{
			...
			if (deviceIsGood && m_WindowSurface)
			{
				...
				else
				{
					...
					if (!surfaceFmts.empty()) // Ensure surfaceFmts is not empty
					{
						for (const vk::SurfaceFormatKHR& surfaceFmt : surfaceFmts)
						{
							// Process each surface format
	...
	bool DeviceManagerVulkan::findQueueFamilies(vk::PhysicalDevice physicalDevice)
	{
	...
	bool DeviceManagerVulkan::createDevice()
	{
		// figure out which optional extensions are supported
		...
		StarEngine::Log::LogMessage(m_DeviceParams.infoLogSeverity, "Enabled Vulkan device extensions:");
		for (const auto& ext : enabledExtensions.device)
		{
			StarEngine::Log::LogMessage(m_DeviceParams.infoLogSeverity, "    %s", ext.c_str());
	...
	#undef APPEND_EXTENSION
		...
		StarEngine::Log::LogMessage(m_DeviceParams.infoLogSeverity, "Created Vulkan device: %s", m_RendererString.c_str());

		return true;
	}

	bool DeviceManagerVulkan::createWindowSurface()
	{
		const VkResult res = glfwCreateWindowSurface(m_VulkanInstance, m_Window, nullptr, (VkSurfaceKHR*)&m_WindowSurface);
		if (res != VK_SUCCESS)
		{
			SE_ERROR("Failed to create a GLFW window surface, error code = %s", nvrhi::vulkan::resultToString(res));
			return false;
		}
	...
	void DeviceManagerVulkan::destroySwapChain()
	{
		if (m_VulkanDevice)
		{
	...
	bool DeviceManagerVulkan::createSwapChain()
	{
		destroySwapChain();
		...
		if (res != vk::Result::eSuccess)
		{
			SE_ERROR("Failed to create a Vulkan swap chain, error code = %s", nvrhi::vulkan::resultToString(VkResult(res)));
			return false;
		}
	...
	bool DeviceManagerVulkan::CreateInstanceInternal()
	{
	...
	bool DeviceManagerVulkan::EnumerateAdapters(std::vector<AdapterInfo>& outAdapters)
	{
		if (!m_VulkanInstance)
			return false;
	...
	void DeviceManagerVulkan::DestroyDeviceAndSwapChain()
	{
		destroySwapChain();
	...
	bool DeviceManagerVulkan::BeginFrame()
	{
	...
	bool DeviceManagerVulkan::Present()
	{
	...
	{
		return new DeviceManagerVulkan();
	}
