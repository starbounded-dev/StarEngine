...
namespace StarEngine {
	...
	Ref<Shader> Shader::Create(const std::string& filepath)
	{
		...
		case RendererAPI::API::Vulkan:  return Ref<VulkanShader>::Create(filepath);
		}
	...
	Ref<Shader> Shader::Create(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc)
	{
		...
		{
			case RendererAPI::API::None:    SE_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
			case RendererAPI::API::Vulkan:  return Ref<VulkanShader>::Create(name, vertexSrc, fragmentSrc);;
		}
